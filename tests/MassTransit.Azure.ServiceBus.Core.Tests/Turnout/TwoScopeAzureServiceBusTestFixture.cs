namespace MassTransit.Azure.ServiceBus.Core.Tests.Turnout
{
    ...
    [TestFixture]
    public class TwoScopeJobConsumerAzureServiceBusTestFixture :
        AzureServiceBusTestFixture
    {
        public TwoScopeJobConsumerAzureServiceBusTestFixture()
        {
        ...
        IBusControl CreateSecondBus()
        {
            return MassTransit.Bus.Factory.CreateUsingAzureServiceBus(x =>
            {
                ...
                x.ServiceInstance(options, instance =>
                {
                    instance.ConfigureJobService();

                    instance.ReceiveEndpoint("crunchthenumbers", e =>
                    {
